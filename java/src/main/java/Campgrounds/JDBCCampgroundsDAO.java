...
public class JDBCCampgroundsDAO implements CampgroundsDAO {
	...
	public List<Campgrounds> getCampgroundById(int parkId) {// Method to select the campgrounds by Park ID
		...
		SqlRowSet returned = jdbcTemplate.queryForRowSet(sqlSearchId, parkId); // SQL search that will use parkId
																				// entered as variable for the where
																				// clause

		while (returned.next()) { // While loop for our SqlRowSet
			Campgrounds aGround = mapRowToGrounds(returned); // Campground object that accesses our mapRowToGrounds
																// method, filling in the column information for each
																// row
			groundsId.add(aGround);// Adding the campground rows into our List
		}
		return groundsId;// When no more campgrounds with that parkId exist, return the list of
							// campgrounds
	}
	...
	private Campgrounds mapRowToGrounds(SqlRowSet returned) { // Method that feeds our SqlRowSet information into a
																// Campground object

		Campgrounds aCampground = new Campgrounds();// Declare an instantiate campground object

		// Use associated setters to set values based on the information passed from our
		// SqlRowSet. Each column in table has a corresponding setter
